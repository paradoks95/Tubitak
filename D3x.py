...
from numpy import float, cumsum, array, abs, where, cos, arange, pi, flipud, unique, append


class Create_Model:
    def __init__(self):
        self.parameter = {"Name": ['SC1','SC2','CL-CH1','CL-CH2','CH1'],
                          "Elastic": [244.8,244.8,612.5,1020,1020],
                          "Poisson": [0.25,0.25,0.25,0.25,0.25],
                          "Density": [1.8,1.9,1.9,1.9,1.9],
                          "Permeability": [1e-06,1e-06,1e-06,1e-06,1e-06],
                          "Void": [1,1,1,1,1],
                          "Thicknesses": [2.5,3.5,4.5,4.5,1.1],
                          "Damping Ratio": 0.02,
                          "Vs": [210,360,550,570,600]}
        ...
        self.Width = float(20) + self.inf_size_x*2
        self.Length = float(20) + self.inf_size_y*2
        self.mesh_size = float(0.2)

        self.ditch_width = float(0.8)
        self.ditch_length = float(5)
        self.ditch2ditch = float(2.2)  # Closest distance
        self.ditch2source = float(2.1)  # Closest distance
        self.ditch_height = float(3.5)
        self.ditch_number = 1

        self.source_size = float(0.8)
        self.accelometer_pattern = [1,3,3,3,3,3]
        self.PGA = 10
        self.duration = 5
        self.frequency = 50
        self.time_step = float(0.002)

        self.model_size = sum(self.accelometer_pattern) + self.source_size
        self.model_name = "Height16"
    ...
    def create_part(self):
        ...
        soilProfileSketch.rectangle(point1=(0, 0), point2=(self.Width, self.Length))
    ...
    def create_section(self):
        for i in range(len(self.layer_heights)):
            ...
            index1 = self.soilPart.cells.findAt((self.Width/2, self.Length/2, z)).index
            index2 = self.soilPart.cells.findAt((self.Width/2, 0.1, z)).index
            index3 = self.soilPart.cells.findAt((self.Width -0.1, self.Length/2, z)).index
            index4 = self.soilPart.cells.findAt((self.Width/2, self.Length - 0.1, z)).index
    ...
    def create_edge_set(self):
        ...
        for z in heights:
            ...
            for node in [F1,F2,F3,F4]:
                horizontal_finite_edges.append(self.soilPart.edges.findAt(node).index)
            ...
            for node in [INF1, INF2, INF3, INF4]:
                horizontal_infinite_edges.append(self.soilPart.edges.findAt(node).index)
        ...
        self.soilPart.Set(edges=self.f(self.soilPart.edges, horizontal_finite_edges), name="H_F_Edges")
        self.soilPart.Set(edges=self.f(self.soilPart.edges, horizontal_infinite_edges), name="H_INF_Edges")
        ...
        for z in heights:
            ...
            for node in [F1, F2, F3, F4,INF1,INF2,INF3,INF4]:
                if z==0.1:
                    ...
                    vertical_edges.append(self.soilPart.edges.findAt(node).index)

        self.soilPart.Set(edges=self.f(self.soilPart.edges, vertical_edges), name="V_Edges")
    ...
    def create_face_sets(self):
        ...
        self.soilPart.Set(faces=self.f(self.soilPart.faces, [X1, X2]), name="X_Faces")
        self.soilPart.Set(faces=self.f(self.soilPart.faces, [Y1, Y2]), name="Y_Faces")
    ...
    def create_datum_planes(self):
        ...
        edge= self.soilPart.edges.findAt((self.Width - self.inf_size_x, self.Length / 2, self.Depth))
        self.soilModel.ConstrainedSketch(gridSpacing=2.37, name='__profile__',
                                         sheetSize=self.Width, transform=self.soilPart.MakeSketchTransform(
                sketchPlane=self.soilPart.faces.findAt((self.Width/2, self.Length/2, self.Depth)),
                sketchPlaneSide=SIDE1, sketchOrientation=RIGHT, origin=(0, 0, self.Depth),sketchUpEdge=edge))

        self.soilPart.projectReferencesOntoSketch(filter=COPLANAR_EDGES, sketch=self.soilModel.sketches['__profile__'])

        self.soilModel.sketches['__profile__'].rectangle(point1=(self.inf_size_x, self.free_space_Y),
                                                         point2=(self.Width - self.inf_size_x, self.free_space_Y + self.source_size))
        self.soilModel.sketches['__profile__'].Line(point1=(self.Width/2, self.inf_size_y), point2=(self.Width/2,self.Length- self.inf_size_y))
        if self.ditch_number > 0:
            self.soilModel.sketches['__profile__'].rectangle(point1=(self.Width/2 - self.ditch_length/2, self.inf_size_y),
                                                             point2=(self.Width/2 + self.ditch_length/2,self.Length - self.inf_size_y))
        self.soilPart.PartitionFaceBySketch(
            faces=self.soilPart.faces.findAt((self.Width / 2, self.Length / 2, self.Depth)),
    ...
    def draw_ditches(self, ditch_number):
        ...
        top_face = self.soilPart.faces.findAt((self.Width/2, self.Length/2, self.Depth))
        edge_index = self.soilPart.edges.findAt((self.Width - self.inf_size_x, self.Length/2, self.Depth)).index
    ...
    def create_mesh(self):
        ...
        # Surface Meshing
        # Y-axis
        self.soilPart.seedEdgeBySize(edges=self.soilPart.sets["SurfaceEdges"].edges,size = self.min_mesh_size)
        self.soilPart.seedEdgeBySize(edges=self.soilPart.sets["HorizontalEdges"].edges,size = self.min_mesh_size)
        ...
        self.PlatePart.seedPart(size=self.min_mesh_size)
        self.PlatePart.generateMesh()
        # self.soilPart.seedEdgeByBias(biasMethod=DOUBLE, endEdges=self.soilPart.sets["TopEdge"].edges, minSize=float(self.min_mesh_size), maxSize=float(self.max_mesh_size))

    def create_nodes(self):
        self.node_list = []
        x2 = self.Width / 2
        y2 = self.free_space_Y + self.source_size + cumsum(self.accelometer_pattern)
        for i in y2:
    ...
    def create_vibration(self):
        ...
        accelerations = self.PGA * 9.81 * cos(2 * pi * self.frequency * time)
    ...
    def create_boundary_conditions(self):
        ...
        self.soilModel.DisplacementBC(createStepName='Initial', name='BC-Z',
                                      region=self.soilModel.rootAssembly.instances["Soil Part-1"].sets['Z_Face'], u1=0,
                                      u2=0, u3=0)
        self.soilModel.DisplacementBC(createStepName='Initial', name='BC-Y',
                                      region=self.soilModel.rootAssembly.instances["Soil Part-1"].sets['Y_Faces'], u2=0)
    ...
    def create_job(self):
        self.job_name = self.model_name.replace("M", "J")
