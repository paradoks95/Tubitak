...
class Create_Model:
    def __init__(self):
        ...
        self.Depth = float(sum(self.parameter["Thicknesses"]))
    ...
    def create_section(self):
        for i in range(len(self.centers)):
            z = float(sorted(self.centers,reverse=True)[i])
    ...
    def create_edge_set(self):
        ...
        if self.ditch_number > 0:
            self.soilPart.PartitionCellByExtrudeEdge(cells=self.soilPart.cells,edges=(
                ...
                self.soilPart.edges.findAt((self.Width*0.5 - self.ditch_length*0.5,self.free_space_Y+self.source_size+self.ditch2source+self.source_size*0.5,self.Depth)),
        ...
        for h in centers:
            cells = [(self.Width*0.5,self.Length-0.1,h),
                    (self.Width-1,self.Length*0.5,h),
                    (self.Width*0.5,0.1,h),
                    (0.1,self.Length*0.5,h)]
